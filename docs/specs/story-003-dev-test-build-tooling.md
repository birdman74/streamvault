...
## Acceptance Criteria
- [ ] AC-1: The Dev persona container image includes Java 21 and Maven (or the Maven wrapper) such that `mvn clean verify` runs successfully against `backend/` with no host intervention
- [ ] AC-2: The Dev persona container image includes Node.js and npm such that the frontend build and test commands run successfully against `frontend/` with no host intervention
- [ ] AC-3: The Test persona container image includes the same Java/Maven and Node/npm tooling as the Dev container, so it can independently run unit tests and Spring MockMvc tests against the implementation
- [ ] AC-4: Running these build/test commands inside either container does not require mounting the host Docker socket or any Docker-in-Docker setup
- [ ] AC-5: `mvn clean verify` runs successfully as a smoke test against the current backend skeleton (from the `chore(STORY-001)` bootstrap commit) inside both containers, with output visible in the persona's session
